...
{
	$buttonBar = new ButtonBar('{list} {create} {update} {print} {delete} {sendnotification}');
	$buttonBar->listUrl = array('index');
	$buttonBar->createUrl = array('create');
	$buttonBar->updateUrl = array('update', 'id'=>$model->so_cd);
	$buttonBar->printUrl = Yii::app()->createUrl('sales/so/preview',array('id'=>$model->so_cd));
...
{
	$buttonBar = new ButtonBar('{list} {create} {update} {print} {sendnotification}');
	$buttonBar->listUrl = array('index');
	$buttonBar->createUrl = array('create');
	$buttonBar->updateUrl = array('update', 'id'=>$model->so_cd);
	$buttonBar->printUrl = Yii::app()->createUrl('sales/so/preview',array('id'=>$model->so_cd));
...
<?php $this->widget('zii.widgets.CDetailView', array(
	...
	'attributes'=>array(
		...
		array('label'=>'Delivery Contact', 'value'=>$model->deliveryContact->contact_name." - ".$model->deliveryContact->position." / ".$model->deliveryContact->phone_no),
		array('label'=>'Delivery Address', 'value'=>$model->deliveryAddress->address),
		...
		array('name'=>'subtotal_cost', 'value'=>'IDR '.number_format($model->subtotal_cost, 2, ".", ",")),
		array('name'=>'subtotal_sell', 'value'=>$model->sell_currency.' '.number_format($model->subtotal_sell, 2, ".", ",")), 
		array('name'=>'subtotal_sell_disc', 'value'=>$model->sell_currency.' '.number_format($model->subtotal_sell_disc, 2, ".", ",")),
		'titipan_currency',
		array('name'=>'total_titipan', 'value'=>$model->titipan_currency.' '.number_format($model->total_titipan, 2, ".", ",")),
		array('label'=>'Total Margin (IDR)','type'=>'number' ,'value'=>(($model->subtotal_sell - $model->subtotal_sell_disc) * $model->sell_kurs) - $model->subtotal_cost - ($model->total_titipan * $model->titipan_kurs)),
		array('name'=>'Total Margin Percent', 'value'=>($model->subtotal_cost!=0) ? number_format(((((($model->subtotal_sell - $model->subtotal_sell_disc) * $model->sell_kurs) - $model->subtotal_cost - ($model->total_titipan * $model->titipan_kurs))/$model->subtotal_cost)*100), 2, ".", ",")." %" : "0%"),
...
<?php $this->widget('zii.widgets.CDetailView', array(
	...
	'attributes'=>array(
		array('label'=>'Invoice Contact', 'value'=>$model->invoiceContact->contact_name." - ".$model->invoiceContact->position." / ".$model->invoiceContact->phone_no),
		array('label'=>'Invoice Address', 'type'=>'raw','value'=>'<b>'.$model->invoiceAddress->short_desc.'</b><br/><br/>'.nl2br($model->invoiceAddress->address)),
		
		array('label'=>'Delivery Contact', 'value'=>$model->deliveryContact->contact_name." - ".$model->deliveryContact->position." / ".$model->deliveryContact->phone_no),
		array('label'=>'Delivery Address', 'type'=>'raw','value'=>'<b>'.$model->deliveryAddress->short_desc.'</b><br/><br/>'.nl2br($model->deliveryAddress->address)),
	),
)); ?>
<br /> <br />
<?php $this->widget('zii.widgets.CDetailView', array(
	...
	'attributes'=>array(
		...
		array('label'=>'Total Margin (IDR)','type'=>'number' ,'value'=>(($model->subtotal_sell - $model->subtotal_sell_disc) * $model->sell_kurs) - $model->subtotal_cost - ($model->total_titipan * $model->titipan_kurs)),
		array('name'=>'Total Margin Percent', 'value'=>($model->subtotal_cost!=0) ? number_format(((((($model->subtotal_sell - $model->subtotal_sell_disc) * $model->sell_kurs) - $model->subtotal_cost - ($model->total_titipan * $model->titipan_kurs))/$model->subtotal_cost)*100), 2, ".", ",")." %" : "0%"),
		array('name'=>'notes','type'=>'raw','value'=>nl2br($model->notes)),
	),
)); ?>
...
<?php $this->widget('zii.widgets.CDetailView', array(
	...
	'attributes'=>array(
		...
		array('name'=>'is_indent','value'=>Status::$is_status[$model->is_indent]),
		array('name'=>'status','value'=>Status::$so_status[$model->status]),
		array('label'=>'Available Status','value'=>Status::$depedency_status[$model->purchase_status]),
		array('name'=>'invoice_status','value'=>Status::$depedency_status[$model->invoice_status]),
		array('name'=>'payment_status','value'=>Status::$depedency_status[$model->payment_status]),
	),
)); ?>
...
$this->widget('zii.widgets.grid.CGridView', array(
		...
		'columns'=>array(
			...
			'cost_currency',
			array('name'=>'cost_price','type'=>'number' ,'value'=>'$data->cost_price','htmlOptions'=>array('class'=>'col-right')),
			array('header'=>'Sell Price - Disc','type'=>'number','value'=>'($data->sell_price - ($data->discount_amt/$data->qty))','htmlOptions'=>array('class'=>'col-right')),
			array('header'=>'Total Cost Price (IDR)','type'=>'number','value'=>'$data->total_cost_price','htmlOptions'=>array('class'=>'col-right')),
			array('header'=>'Total Sell Price - Disc','type'=>'number','htmlOptions'=>array('class'=>'col-right'),'value'=>'$data->total_sell_price - $data->discount_amt'),
