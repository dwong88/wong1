...
/**
 * This is the model class for table "tcurrency".
 *
 * The followings are the available columns in table 'tcurrency':
...
class Currency extends ActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('currency_cd, currency_name', 'required'),
			array('currency_cd', 'length', 'max'=>5),
			array('currency_name', 'length', 'max'=>20),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('currency_cd, currency_name', 'safe', 'on'=>'search'),
		);
	}
